...
from Connector.LogicSSA import ConnectorSSA
...
class SSATable(QWidget):
    def __init__(self, db_name, tab_project, method):
        ...
        self.method = method
        self.__connector_ssa__ = ConnectorSSA(db_name)
    ...
    def Insert(self):
        ...
        self.__connector_ssa__.Insert(number)
    ...
    def Delete(self):
        ...
        for row in self.checked:
            ...
            self.__connector_ssa__.Delete(number)
    ...
    def Save(self):
        ...
        for row, col in edited:
            ...
            if col == 2:
                ...
                self.__connector_ssa__.Update(number, '논리번호', value)
            elif col == 3:
                value = self.table.item(row, col).text()
                self.__connector_ssa__.Update(number, '영향기능', value)
            elif col == 4:
                value = self.table.item(row, col).text()
                self.__connector_ssa__.Update(number, '기기조합', value)
            elif col == 5:
                value = self.table.item(row, col).text()
                self.__connector_ssa__.Update(number, '비고', value)
        self.edited.clear()
        self.method()
