...
class EquipTab(QWidget):
    ...
    def __variables__(self):
        self.columns = ['선택']+self.__return_equip__.Columns()
        self.dataframe = self.__return_equip__.Dataframe()
        self.checked = []
        self.edited = []
    ...
    def Insert(self):
        ...
        self.__connector_equip__.Insert(number)
    ...
    def Delete(self):
        ...
        for row in self.checked:
            ...
            self.__connector_equip__.Delete(number)
    ...
    def Save(self):
        ...
        for row, col in edited:
            ...
            if col == 2:
                ...
                self.__connector_equip__.Update(number, '기기번호', value)
            elif col == 3:
                value = self.table.item(row, col).text()
                self.__connector_equip__.Update(number, '기기명', value)
            elif col == 4:
                value = self.table.item(row, col).text()
                self.__connector_equip__.Update(number, '방화지역', value)
            elif col == 5:
                value = self.table.item(row, col).text()
                self.__connector_equip__.Update(number, '계열', value)
            elif col == 6:
                value = self.table.item(row, col).text()
                self.__connector_equip__.Update(number, '비고', value)
        self.edited.clear()
        self.method()
    ...
    def __layout__(self):
        layout = QVBoxLayout()
        layout.addWidget(QLabel('※ 기기정보 정보\n'))
